...
function rewriteFile(file) {
    ...
    for (const statement of ast.program.body) {
        if ((statement.type === 'ImportDeclaration' || statement.type === 'ExportNamedDeclaration' || statement.type === 'ExportAllDeclaration') && statement.source && statement.source.value.startsWith('types/')) {
            const source = statement.source
            const absoluteTypePath = path.resolve(typesDir, source.value.slice(6))
            const relativeTypePath = slash(path.relative(path.dirname(file), absoluteTypePath))
            // @ts-ignore
